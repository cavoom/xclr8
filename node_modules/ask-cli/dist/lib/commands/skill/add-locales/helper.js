...
function addLocales(selectedLocales, profile, doDebug, callback) {
    ...
    _getNewLocaleModelUri(selectedLocales, profile, doDebug, (resolveErr, iModelSourceByLocales) => {
        ...
        const iModelFolderPath = path.join(process.cwd(), ResourcesConfig.getInstance().getSkillMetaSrc(profile), CONSTANTS.FILE_PATH.SKILL_PACKAGE.INTERACTION_MODEL, "custom");
        const remoteTemplateSet = new Set([...iModelSourceByLocales.values()].reduce((acc, cur) => {
            if (!cur.canCopy) {
                acc.push(cur.uri);
            }
            return acc;
